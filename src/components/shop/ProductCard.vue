...
<template>
  <article class="group flex h-full flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition-all duration-300 hover:shadow-lg hover:-translate-y-1 dark:border-slate-700 dark:bg-slate-800">
    <RouterLink
      :to="{ name: 'product-details', params: { slug: product.slug } }"
      class="relative block aspect-[4/3] overflow-hidden bg-slate-100 dark:bg-slate-700"
    >
      <img
        :src="image"
        :alt="product.name"
    ...
    <div class="flex flex-1 flex-col p-5">
      <p class="text-xs font-semibold text-sky-600 uppercase tracking-wide dark:text-sky-400">
        {{ product.category?.name ?? 'Computer gear' }}
      </p>
      <RouterLink
        :to="{ name: 'product-details', params: { slug: product.slug } }"
        class="mt-2 line-clamp-2 min-h-12 text-base font-bold leading-6 text-slate-900 transition-colors hover:text-sky-600 dark:text-slate-100 dark:hover:text-sky-400"
      >
        {{ product.name }}
      </RouterLink>
      ...
      <div class="mt-4 flex items-end gap-2">
        <p class="text-xl font-bold text-slate-900 dark:text-slate-100">{{ money(finalPrice) }}</p>
        <p
          v-if="discount"
          class="pb-0.5 text-sm font-semibold text-slate-400 line-through dark:text-slate-500"
        >
          {{ money(product.price) }}
        </p>
      </div>
      ...
      <div class="mt-5 grid grid-cols-[1fr_44px] gap-2">
        <button
          type="button"
          class="inline-flex items-center justify-center gap-2 rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white shadow-sm transition-all hover:bg-slate-800 disabled:opacity-50 disabled:cursor-not-allowed dark:bg-sky-600 dark:hover:bg-sky-700"
          :disabled="!inStock"
        ...
        <RouterLink
          :to="{ name: 'product-details', params: { slug: product.slug } }"
          class="inline-flex items-center justify-center rounded-lg border border-slate-300 bg-white px-2.5 py-2.5 text-slate-600 shadow-sm transition-all hover:bg-slate-50 hover:border-slate-400 dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600 dark:hover:border-slate-500"
